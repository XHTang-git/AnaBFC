...
class Visualizer():
    ...
    def save_images(self, webpage, visuals, image_path, id):
        ...
        name = os.path.splitext(short_path)[0]
        source_image=sitk.ReadImage("E:\WBFC_2\mri_image\down_source\\"+name+".mhd")

        webpage.add_header(name)
        shape = source_image.GetSize()
        ...
        for label, image_numpy in visuals.items():
            ...
            image_numpy = image_numpy[0:shape[2], 0:shape[1], 0:shape[0]]
            volout = sitk.GetImageFromArray(image_numpy)
            volout.CopyInformation(source_image)
            sitk.WriteImage(volout, image_dir + '/' + name +label+ '.nii.gz')
            idd=idd+1


    def save_mr300(self, webpage, visuals, image_path, id):
        image_dir = webpage.get_image_dir()
        short_path = ntpath.basename(image_path[0])
        ...
        for label, image_numpy in visuals.items():
            if label=="fake_U" or label=="fake_B":
                image_name = '%s_%s' % (id, label)
